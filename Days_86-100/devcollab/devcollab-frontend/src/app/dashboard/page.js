  ...
  useEffect(() => {
    async function fetchDashboardData() {
      ...
      try {
        ...
        const requestPromises = myProjects
          .map((project) => getProjectRequests(project.id))
          .then((requests) =>
...
//   Delete the project
  async function handleDeleteProject(projectId, projectTitle) {
    if (!window.confirm(`Are you sure you want to delete "${projectTitle}"? This will also delete all collaboration requests for this project.`)) {
      return
    }
    ...
    try {
      ...
      setAllRequests((prev) => prev.filter((r) => r.projectId !== projectId))
          ...
          <div className="max-w-5xl mx-auto px-4 py-8">
            ...
            <div className="mb-8">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-sm text-gray-500">{totalProjects} project{totalProjects !== 1 ? 's' : ''}</span>
              </div>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                          <div className="flex-1 min-w-0">
                            <div className="flex flex-wrap items-center gap-2 mb-2">
                              ...
                              <span className={`text-xs font-medium px-2.5 py-0.5 rounded-full ${statusBadge(project.status)}`}>
