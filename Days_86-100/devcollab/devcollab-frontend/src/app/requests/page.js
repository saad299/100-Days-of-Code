...
function MyRequestsPage() {
  ...
  const getStatusBadge = (status) => {
    ...
    switch (status) {
      ...
      case 'rejected':
        return <span className={`${base} bg-red-100 text-red-700`}>Not Selected</span>;
      default:
      ...
      <div>
        {requests.length === 0 && !error ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <p className="text-gray-500 mb-4">You haven&apos;t sent any collaboration requests yet.</p>
            <Link
              href="/projects"
          ...
          <div className="overflow-x-auto rounded-lg border border-gray-200">
            <table className="min-w-full divide-y divide-gray-200 text-sm">
              <thead className="bg-gray-50">
                <tr>
                  {['Project', 'Owner', 'Tech Stack', 'Date Sent', 'Message', 'Status'].map(
                    (col) => (
                      <th
                        key={col}
                  ...
                  <tr key={request.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex flex-wrap gap-1">
                        {request.project_detail.tech_stack_list.slice(0, 3).map((tech) => (
                          <span
                            key={tech}
